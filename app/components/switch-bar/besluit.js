...
export default class SwitchBarBesluitComponent extends Component {
    @tracked selected;
    @tracked eenheids;
    @tracked heids;
    @service map;;
...
{ 
  ...
  ORDER BY DESC(?geplandeStart) xsd:integer(?title) ASC(?title)`;
    ...
    // get 2 data query in one
      ...
      /* // could not start with the date getaanwezig cause it's not every meeting has record aanwezig data.
      getAanwezig.then( results => {
      const aanwezigenBijStart = []
      const dataStemming = []
      results.results.bindings.forEach(e => {
      // console.log(e)
      getData.then( results => {
      results.results.bindings.forEach(l => {
          // console.log(l);
          if(e.geplandeStart.value == l.geplandeStart.value){
              dataStemming.push({
              geplandeStart: l.geplandeStart.value,
              bestuursclassificatie: l.bestuursclassificatie.value,
              location: l.location,
        ...
        this.heids = {aanwezigenBijStart,dataStemming}
      })}) */
    }// end get besluit
}
